...
trying to make need not to be last complete period

t_min< number_sample_rate/frequency_object)<t_max
...
4.need samples with greater change during each time interval
...
def segment(inp,u):
    ...
    for i in range(1,len(inp)+1):
        ...
        for itr in range(4,20):
            if 3*t_min<= i <=3*t_max:
            ...
            if i==len(inp) and itr*t_max > i:
                temp2 =np.full(itr*t_max-itr*t_min+1,2e21)
                for last in range(itr*t_min,len(inp)-1):
                    #print last, i-1
...
"---------------iterations -------------------------------------------------"
#Possibilities:
#1.values can oscillate between 2 values. 
def main(inp):
#    inp =der(inp)
    u = np.zeros(t_min)
    u=inp[0:t_max-1]
    ...
    for i in range(0,8):
        ...
        #plt.plot(u)
        #plt.show()
        if abs(cost[i+1] - cost[i]+1)/(cost[i]+cost[i+1]+1) < 0.5:
            freq =0.000
...
'''
            if i==len(inp) and (itr-1)*t_max< len(inp) < itr*t_min:
                temp2=np.full(itr*t_max-itr*t_min+1,2e21)
                for lst in range((itr-1)*t_min,(itr-1)*t_max+1):
                    temp2[lst-(itr-1)*t_min] = d[lst]+ var_seg(inp[lst:len(inp)-1],u)
                #d[len(inp)]=np.amin(temp)
                print itr ,'1st if'
                s[len(inp)]=np.append(s[np.argmin(temp2)+(itr-1)*t_min],np.argmin(temp2)+(itr-1)*t_min)
