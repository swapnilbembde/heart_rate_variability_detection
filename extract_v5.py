...
This is with maximum variation in data because of data_min =0
    ...
    for i in range(1,len(inp)+1):
        ...
        for itr in range(4,20):
            ...
            if i==len(inp) and (itr-1)*t_max< len(inp) < itr*t_min:
                ...
                print itr
                s[len(inp)]=np.append(s[np.argmin(temp)+(itr-1)*t_min],np.argmin(temp)+(itr-1)*t_min)
                temp2=np.full(t_max-t_min+1,2e21)
                return s[i]
